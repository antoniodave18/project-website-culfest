...
export async function GET() {
    try {
        ...
        return NextResponse.json({ success: false, message: 'Gagal mengambil data kandidat' }, { status: 500 });
    }
}
...
export async function POST(request: Request) {
    try {
        ...
        if (!name || !imageBase64) {
            return NextResponse.json({ success: false, message: 'Nama dan gambar wajib diisi' }, { status: 400 });
        }
        ...
        return NextResponse.json({ success: false, message: 'Gagal menyimpan kandidat' }, { status: 500 });
    }
}
